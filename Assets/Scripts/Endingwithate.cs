...
public class Endingwithate : MonoBehaviour
{
...
//finalscore
    ...
    public void Start()
    {
      ...
      public void GivefirstWord(){
        ...
        int i = random.Next(0, 800);
        fword.text = Constant11.ewate[i];
        temp.Add(Constant11.ewate[i]);
...
public void LevelEleven()
    ...
    {   
        ...
        int n = s.Length;
        if(Constant11.ewate.Contains(s))
        {
            for (int i = 0; i < n; i++)
            {
                if ((s[n-1]=='e')&&(s[n-2]=='t')&&(s[n-3]=='a'))
                {
                    if(temp.Contains(s))
                    {                    
                    ...
                    else
                    {
                        ...
                        else if (correctAns == 2)
                        {
                            ...
                            int flag=PlayerPrefs.GetInt("Attemptedornot11",0);
                            if(flag==0)
                            {
                                if (tempf <= 5)
                                {
                                ...
                                PlayerPrefs.SetInt("Attemptedornot11",1);
                            }
                            else
                            {
                                currentscore=0;
                            ...
                            if(currentlevel<=12)
                            {
                                PlayerPrefs.SetInt("levelsunlocked",12);
                            }
                            else
                            {
